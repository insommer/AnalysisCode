...
Lists=[]
Lists.append( '''#ZS Bias (A)
6
''')
Lists.append('''#freq (MHz)
225.5
225.6
225.7
225.8
...
for ind, List in enumerate(Lists):
    variable = List.split("\n")[0][1:]
    list_file_name ="Variable "+ str(ind)+".txt"
    ...
    if not os.path.exists(list_file):
        with open(list_file,"w") as f:
            f.write(List)
    #then load
    cicero_values.append( np.loadtxt(list_file))
    variables.append(variable)
    
print(cicero_values)

list_lengths = [np.size(li) for li in cicero_values ]
print(list_lengths)
# import sys
# sys.exit()
...
for ind in range(imgNo):
    ...
    if new_figure and ind>0:
        plt.savefig(data_folder+"/"+ str(ind-NUM_ROWS)+" to "+str(ind-1)+".png")
        plt.close()
